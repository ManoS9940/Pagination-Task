var list = [
  ...
  function createobj({name,id,email}){
document.querySelector(".list").innerHTML += `<div class="list-container">
        <div>
...
for(let i =1;i<=Pages;i++){
  ...
  page.onclick=() =>{
    ...
    const Users = list.slice( (i-1)*10 ,i*10)
    document.querySelector(".list").innerHTML = "";
    Users.forEach((object) => createobj(object));
}
  }
  const firstTenObj = list.slice(0,10);
  firstTenObj.forEach((object) => createobj(object));